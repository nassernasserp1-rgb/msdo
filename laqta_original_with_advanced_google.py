...
# -*- coding: utf-8 -*-
"""
LAQTA - الواجهة القديمة مع النظام المتقدم لجوجل
الواجهة الأصلية + الاستخراج المتقدم من جوجل + إرسال الصور
"""
...
google_comparison_enabled = [True]  # النظام المتقدم
...
# نظام الاستخراج المتقدم من جوجل (مبسط للواجهة القديمة)
class OriginalAdvancedGoogleExtractor:
    """مستخرج متقدم للواجهة الأصلية"""
    ...
    def clean_for_google_search(self, product_name: str) -> str:
        """تنظيف اسم المنتج للبحث في جوجل"""
        
        # إزالة الكلمات غير المهمة
        unwanted = ['amazon', 'choice', 'brand', 'original', 'authentic']
        ...
        for word in clean_name.split():
            if (len(word) > 3 and 
                word.lower() not in unwanted and 
                not word.isdigit()):
                words.append(word)
            if len(words) >= 4:  # أول 4 كلمات مهمة
                break
        
        search_term = ' '.join(words) + " سعر مصر"
        return search_term.strip()
    
    async def google_price_comparison(self, product_name: str, amazon_price: float) -> dict:
        """مقارنة الأسعار عن طريق جوجل - نسخة مبسطة للواجهة الأصلية"""
        
        search_term = self.clean_for_google_search(product_name)
        cache_key = f"original_google_{search_term}_{amazon_price}"
            ...
            async with async_playwright() as p:
                ...
                google_data = await page.evaluate("""
                    () => {
                        ...
                        // استخراج المواقع
                        const sites = ['amazon.eg', 'noon.com', 'jumia.com', 'carrefour', 'souq.com', 'btech'];
                        ...
                        // إزالة التكرار
                        data.prices = [...new Set(data.prices)].sort((a, b) => a - b);
                ...
                if google_data['prices'] and len(google_data['prices']) >= 2:
                    prices = google_data['prices']
                    sites = google_data['sites']
                    
                    result['market_prices'] = prices
                    result['market_sites'] = sites
                    ...
                    print(f"   📊 {total_competitors} أسعار، متوسط {avg_price:,.0f}، ترتيب {amazon_rank}")
                    print(f"   {result['reason']}")
                    
                    self.stats['successful_extractions'] += 1
                
        except Exception as e:
            print(f"   ❌ خطأ في جوجل: {e}")
        
        self.stats['total_searches'] += 1
        self.cache[cache_key] = result
        return result

# إنشاء مستخرج جوجل للواجهة الأصلية
original_google_extractor = OriginalAdvancedGoogleExtractor()

def send_telegram_alert(item, old_price, new_price, discount_percent, drop_detected):
    """إرسال تنبيه تليجرام مع الصورة والمقارنة المتقدمة"""
    
    def google_compare_and_send():
        """مقارنة جوجل وإرسال مع الصورة"""
        ...
        if google_comparison_enabled[0]:
            ...
            try:
                comparison_result = loop.run_until_complete(
                    original_google_extractor.google_price_comparison(item.get('name', ''), new_price)
                )
                ...
                if not comparison_result['is_good_deal'] and comparison_result['confidence'] < 50:
                    ...
                    original_google_extractor.stats['rejected_deals'] += 1
                    return
                ...
                original_google_extractor.stats['validated_deals'] += 1
...
def send_actual_telegram_alert(item, old_price, new_price, discount_percent, drop_detected):
    """إرسال تنبيه مع الصورة ومعلومات جوجل"""
    try:
        ...
        if google_confidence >= 80:
            headline = "🔥 <b>BEST DEAL VERIFIED!</b> 🔥"
...
🤖 <b>Advanced Google Price Comparison</b>
"""
...
def add_alert_data(item, old_price, new_price, discount_percent, drop_detected=False):
    """إضافة بيانات التنبيه مع المقارنة المتقدمة"""
    asin = item.get("asin")
...
async def scrape_single_page(section, section_url, page_num, db, log_fn=None, discount_alert_cb=None, discount_threshold=25):
    ...
    async with async_playwright() as p:
        ...
        if log_fn:
            google_mode = "[GOOGLE]" if google_comparison_enabled[0] else ""
...
def start_scraping():
    ...
    log(f"🟢 Starting - New Products: {auto_mode}, Google: {google_mode}")
...
def show_stats():
    ...
    # إحصائيات جوجل
    if google_comparison_enabled[0]:
        stats = original_google_extractor.stats
        log(f"🔍 Google Stats:")
        log(f"   📊 Searches: {stats['total_searches']}")
        log(f"   ✅ Successful: {stats['successful_extractions']}")
        log(f"   📱 Validated: {stats['validated_deals']}")
        log(f"   🚫 Rejected: {stats['rejected_deals']}")
        log(f"   🧠 Cache: {stats['cache_hits']}")
...
def toggle_google_comparison():
    google_comparison_enabled[0] = not google_comparison_enabled[0]
...
# ==== الواجهة الأصلية ====
root = ctk.CTk()
root.title("LAQTA - Original Interface with Advanced Google")
...
subtitle_label = ctk.CTkLabel(root, text="Amazon Egypt Products Scraper with Advanced Google Comparison", 
...
google_chk = ctk.CTkCheckBox(controls_frame, text="🔍 Google Compare", font=("Arial", 13), 
                            text_color="#4285f4", command=toggle_google_comparison)
google_chk.grid(row=0, column=4, padx=5, pady=8, sticky="ew")
google_chk.select()  # مفعل افتراضياً
...
# رسائل ترحيب أصلية
log("🚀 LAQTA started!", "🟢")
log("🔍 Google Comparison: ON - Advanced extraction enabled", "✨")
log("📱 Telegram: ON - with photos and market analysis", "📸")
log("🆕 Auto New: OFF - all products", "📦")
log("📊 Expected: High-quality deals with Google verification!", "🏆")
