...
# -*- coding: utf-8 -*-
"""
LAQTA Gemini - مقارنة أسعار شاملة مع Google Gemini AI
نفس النتائج اللي شفتها في Gemini - مدمجة في النظام!
"""
...
class GeminiMarketComparator:
    ...
    def __init__(self):
        ...
        if self.ai_enabled:
            ...
            print("⚠️ Google Gemini غير مفعل")
            print("💡 للحصول على مقارنة شاملة:")
            print("   1. احصل على API مجاني: https://makersuite.google.com/app/apikey")
...
يرجى الاجابة بالعربية وبشكل مختصر ومنظم."""
    ...
    def parse_gemini_response(self, response: str, amazon_price: float) -> Dict:
        ...
        if any(word in response_lower for word in ['ممتاز', 'جيد جداً', 'اشتري فوراً']):
            recommendation = "اشتري فوراً"
...
"""
        ...
        reply_markup = {
            "inline_keyboard": [
                ...
                [
                    {"text": "🌙 Noon", "url": f"https://www.noon.com/egypt-en/search/?q={urllib.parse.quote(search_term)}"},
                    {"text": "🛒 Jumia", "url": f"https://www.jumia.com.eg/catalog/?q={urllib.parse.quote(search_term)}"}
                ],
...
def create_gemini_config():
    """إنشاء ملف إعداد Gemini"""
    
    config = {
        "gemini_api_key": "",
        "instructions": {
            "step1": "احصل على API مجاني من: https://makersuite.google.com/app/apikey",
            "step2": "اضغط 'Create API Key'",
            "step3": "انسخ الـ key وضعه في gemini_api_key",
            "step4": "احفظ الملف"
        },
        "benefits": {
            "comprehensive": "مقارنة شاملة مع جميع المواقع المصرية",
            "organized": "نتائج منظمة: اسم الموقع | السعر | حالة التوفر",
            "intelligent": "تحليل ذكي وتوصيات احترافية",
            "free": "مجاني - 60 طلب/دقيقة",
            "reliable": "أكثر استقراراً من Groq"
        }
    }
    ...
    log("✅ تم إنشاء gemini_config.json", "📁")
    log("💡 ضع API key من: https://makersuite.google.com/app/apikey", "💡")

# الواجهة البسيطة
root = ctk.CTk()
root.title("LAQTA Gemini - Comprehensive Market Comparison")
...
subtitle_label = ctk.CTkLabel(root, text=f"{gemini_status} - مقارنة شاملة مع السوق المصري", 
...
test_btn = ctk.CTkButton(buttons_frame, text="🧪 Test Gemini Comparison", 
...
config_btn = ctk.CTkButton(buttons_frame, text="⚙️ Setup Gemini", 
                          command=create_gemini_config, fg_color="#2196F3")
...
if gemini_comparator.ai_enabled:
    ...
    log("⚠️ للحصول على مقارنة شاملة مع Gemini:", "💡")
    log("   1. اضغط 'Setup Gemini' لإنشاء ملف الإعداد", "💡")
    log("   2. احصل على API مجاني: https://makersuite.google.com/app/apikey", "💡")
    log("   3. ضع API key في gemini_config.json", "💡")

log("🧪 اضغط 'Test Gemini Comparison' لاختبار المقارنة الشاملة", "🎯")
