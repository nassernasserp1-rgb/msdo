...
# -*- coding: utf-8 -*-
"""
واجهة المستخدم المحسنة مع دعم ملف JSON القديم
"""
...
class OptimizedAppWithJSON:
    ...
    def setup_ui(self):
        """إعداد واجهة المستخدم المحسنة مع دعم JSON"""
        self.root = ctk.CTk()
        self.root.title("LAQTA - Optimized with JSON Support")
        ...
        title_label = ctk.CTkLabel(
            self.root, 
            text="LAQTA - JSON COMPATIBLE", 
            font=("SST Arabic Medium", 55), 
            text_color="#54fac8"
        )
        ...
        self.progress_bar.grid(row=3, column=0, padx=10, pady=7, sticky="ew")
        ...
        self.log_textbox.grid(row=5, column=0, padx=15, pady=(0, 12), sticky="nsew")
    ...
    def setup_json_frame(self):
        """إعداد إطار ملف JSON"""
        json_frame = ctk.CTkFrame(self.root, fg_color="#1a1f2b", height=70)
        ...
        json_label = ctk.CTkLabel(
            json_frame, 
            text="📁 ملف JSON القديم:", 
            font=("Arial", 16, "bold"), 
            text_color="#54fac8"
        )
        json_label.grid(row=0, column=0, padx=10, pady=15, sticky="w")
        ...
        self.json_path_label.grid(row=0, column=1, padx=10, pady=15, sticky="w")
        
        # زر اختيار الملف
        select_json_btn = ctk.CTkButton(
            json_frame, 
        ...
        select_json_btn.grid(row=0, column=2, padx=10, pady=15)
        
        # زر تحويل JSON إلى SQLite
        convert_btn = ctk.CTkButton(
            json_frame, 
            text="🔄 تحويل إلى SQLite", 
            command=self.convert_json_to_sqlite,
            font=("Arial", 14, "bold"),
            fg_color="#59ff9d", 
            hover_color="#13e6a7", 
            text_color="#111927",
        ...
        convert_btn.grid(row=0, column=3, padx=10, pady=15)
        
        # معلومات الملف
        self.json_info_label = ctk.CTkLabel(
            json_frame, 
            text="", 
            font=("Arial", 12), 
            text_color="#59ff9d"
        )
        self.json_info_label.grid(row=1, column=1, columnspan=3, padx=10, pady=(0, 10), sticky="w")
    ...
    def setup_stats_frame(self):
        ...
        self.stats_frame.grid_columnconfigure((0,1,2,3,4,5,6), weight=1)
        ...
        for i, (name, key) in enumerate(stats_names):
            ...
            frame.grid(row=0, column=i, padx=3, pady=10, sticky="ew")
            
            title = ctk.CTkLabel(frame, text=name, font=("Arial", 11, "bold"), text_color="#54fac8")
            title.pack()
            
            value = ctk.CTkLabel(frame, text="0", font=("Arial", 15, "bold"), text_color="#ffffff")
            value.pack()
    ...
    def setup_buttons(self):
        ...
        buttons_frame.grid(row=6, column=0, padx=10, pady=10, sticky="ew")
        ...
        self.start_btn = ctk.CTkButton(
            buttons_frame, text="🚀 Start", command=self.start_scraping,
        ...
        self.alerts_btn = ctk.CTkButton(
            buttons_frame, text="📢 Alerts", command=self.open_alerts_window,
            width=btn_w, height=btn_h, font=btn_font, fg_color="#59ff9d", 
            hover_color="#12dafb", text_color="#111927"
        )
        self.alerts_btn.grid(row=0, column=2, padx=5, pady=8, sticky="ew")
        ...
        self.export_btn.grid(row=0, column=3, padx=5, pady=8, sticky="ew")

        self.migrate_btn = ctk.CTkButton(
            buttons_frame, text="🔄 Migrate", command=self.migrate_json,
            width=btn_w, height=btn_h, font=btn_font, fg_color="#ffa726", 
            hover_color="#ff9800", text_color="#111927"
        )
        self.migrate_btn.grid(row=0, column=4, padx=5, pady=8, sticky="ew")

        self.clear_btn = ctk.CTkButton(
            buttons_frame, text="🧹 Clear", command=self.clear_log,
            width=btn_w, height=btn_h, font=btn_font, fg_color="#9c27b0", 
            hover_color="#7b1fa2", text_color="#ffffff"
        )
        self.clear_btn.grid(row=0, column=5, padx=5, pady=8, sticky="ew")
        ...
        self.exit_btn.grid(row=0, column=6, padx=5, pady=8, sticky="ew")
    ...
    def select_json_file(self):
        ...
        if file_path:
            ...
            try:
                ...
                info_text = f"{products_count:,} منتج | {file_size:.1f} MB"
                self.json_info_label.configure(text=info_text)
                
                # تحديث إحصائية JSON
                self.stats_labels["json_products"].configure(text=str(products_count))
                
                self.log(f"✅ تم تحديد ملف JSON: {file_name}")
                self.log(f"📊 يحتوي على {products_count:,} منتج")
                
            except Exception as e:
                self.log(f"❌ خطأ في قراءة ملف JSON: {e}")
                messagebox.showerror("خطأ", f"لا يمكن قراءة ملف JSON:\n{e}")

    def convert_json_to_sqlite(self):
        """تحويل JSON إلى SQLite"""
        if not self.json_file_path:
            messagebox.showwarning("تحذير", "يرجى اختيار ملف JSON أولاً")
            return
        
        # تشغيل أداة التحويل في thread منفصل
        def convert_thread():
            try:
                from migrate_json_to_db import migrate_json_to_sqlite
                success = migrate_json_to_sqlite(self.json_file_path)
                
                if success:
                    self.log("✅ تم تحويل JSON إلى SQLite بنجاح!")
                    messagebox.showinfo("نجح", "تم تحويل البيانات بنجاح!")
                else:
                    self.log("❌ فشل في تحويل JSON")
                    messagebox.showerror("خطأ", "فشل في تحويل البيانات")
                    
            except Exception as e:
                self.log(f"❌ خطأ في التحويل: {e}")
                messagebox.showerror("خطأ", f"خطأ في التحويل:\n{e}")
        
        self.log("🔄 بدء تحويل JSON إلى SQLite...")
        threading.Thread(target=convert_thread, daemon=True).start()

    def migrate_json(self):
        """تشغيل أداة التحويل"""
        self.convert_json_to_sqlite()
    ...
    async def run_scraper(self, section, pages, all_pages, concurrency):
        """تشغيل السكرابر المحسن مع دعم JSON"""
    ...
    def start_scraping(self):
        ...
        json_status = "مع دعم JSON" if self.json_file_path else "بدون JSON"
        self.log(f"🚀 بدء السكرابة المحسنة {json_status} - القسم: {section}, الصفحات: {pages}, التزامن: {concurrency}", "🎯")
    ...
    def update_stats_periodically(self):
        ...
        if self.running:
            self.update_stats()
            self.root.after(2000, self.update_stats_periodically)
    ...
    def export_data(self):
        ...
        if self.scraper:
            ...
            export_data = {
                ...
                "json_file_used": self.json_file_path
            }
    ...
    def run(self):
        """تشغيل التطبيق"""
        self.log("🎯 LAQTA Optimized with JSON Support started!", "🚀")
        self.log("💡 Features: JSON compatibility, SQLite DB, concurrent scraping", "ℹ️")
        self.log("📁 يمكنك اختيار ملف JSON القديم لاستكمال العمل عليه", "💡")
        self.root.mainloop()

if __name__ == "__main__":
    app = OptimizedAppWithJSON()
    app.run()