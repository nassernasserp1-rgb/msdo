...
# -*- coding: utf-8 -*-
"""
LAQTA مع نظام مقارنة الأسعار الخارجية - مُصلح
"""
...
# نظام مقارنة الأسعار المبسط والسريع
class SimplePriceComparator:
    """مقارن أسعار مبسط وسريع"""
    ...
    def clean_search_term(self, product_name: str) -> str:
        ...
        # أخذ أهم الكلمات فقط
        unwanted = ['amazon', 'choice', 'pack', 'piece', 'brand']
        clean_name = re.sub(r'[^\w\s]', ' ', product_name.lower())
        words = [word for word in clean_name.split() if word not in unwanted and len(word) > 2]
        return ' '.join(words[:3])  # أول 3 كلمات فقط
    
    async def quick_jumia_check(self, product_name: str, amazon_price: float) -> dict:
        """فحص سريع في جوميا فقط (للسرعة)"""
        
        search_term = self.clean_search_term(product_name)
        cache_key = f"jumia_{search_term}_{amazon_price}"
        ...
        result = {
            ...
            'is_good_deal': False,
            'confidence': 30,
            'reason': 'لم يتم العثور على مقارنة'
        }
        ...
        try:
            async with async_playwright() as p:
                ...
                await page.goto(jumia_url, timeout=8000)
                await page.wait_for_timeout(1500)
                
                # استخراج أول سعر فقط (للسرعة)
                first_price = await page.evaluate("""
                    () => {
                        const priceElements = document.querySelectorAll('.prc, .-prc, .price');
                ...
                if first_price:
                    result['external_price'] = first_price
                    price_diff = ((first_price - amazon_price) / first_price) * 100
                    result['price_difference'] = price_diff
                    
                    # تحديد جودة العرض
                    if price_diff > 15:  # أمازون أرخص بأكثر من 15%
                        result['is_good_deal'] = True
                        result['confidence'] = 90
                        result['reason'] = f"🔥 أرخص بـ {price_diff:.0f}% من جوميا"
                        
                    elif price_diff > 5:   # أرخص بأكثر من 5%
                        result['is_good_deal'] = True
                        result['confidence'] = 75
                        result['reason'] = f"✅ أرخص بـ {price_diff:.0f}% من جوميا"
                        
                    elif price_diff > -10:  # فرق بسيط
                        result['is_good_deal'] = True
                        result['confidence'] = 60
                        result['reason'] = f"⚠️ سعر مقارب لجوميا ({price_diff:+.0f}%)"
                        
                    else:  # أغلى بكثير
                        result['is_good_deal'] = False
                        result['confidence'] = 25
                        result['reason'] = f"❌ أغلى بـ {abs(price_diff):.0f}% من جوميا"
                    
                    self.comparison_stats['successful_comparisons'] += 1
                    print(f"   📊 جوميا: {first_price:,.0f} vs أمازون: {amazon_price:,.0f} ({price_diff:+.0f}%)")
                
        except Exception as e:
            print(f"   ⚠️ خطأ في فحص جوميا: {e}")
...
# إنشاء مقارن الأسعار
price_comparator = SimplePriceComparator()
...
def send_telegram_alert(item, old_price, new_price, discount_percent, drop_detected):
    ...
    def compare_and_send():
        ...
        # مقارنة الأسعار إذا كانت مفعلة
        if price_comparison_enabled[0]:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            
            try:
                print(f"🔍 فحص سعر: {item.get('name', '')[:40]}...")
                
                comparison_result = loop.run_until_complete(
                    price_comparator.quick_jumia_check(item.get('name', ''), new_price)
                )
                
                if not comparison_result['is_good_deal']:
                    print(f"🚫 تم رفض التنبيه: {comparison_result['reason']}")
...
def send_actual_telegram_alert(item, old_price, new_price, discount_percent, drop_detected):
    ...
    try:
        ...
        # إضافة معلومات المقارنة
        comparison_row = f"\n🔍 <b>vs Market:</b> {comparison_info}" if comparison_info else ""
...
"""
        ...
        reply_markup = {
            "inline_keyboard": [
                ...
                [{"text": "🔍 Compare on Jumia", "url": f"https://www.jumia.com.eg/catalog/?q={product_name.replace(' ', '+')}"}]
            ]
        }
    ...
    async with async_playwright() as p:
        ...
        if auto_new_products_mode[0]:
            ...
            if random.choice([True, False]):
                ...
                random_page = random.randint(page_num, page_num + 15)
        ...
        for item in items:
            try:
                ...
                if strike_price and price and strike_price > price:
                    ...
                    if discount_percent >= discount_threshold and discount_percent <= 90 and price >= 20:
...
async def scrape_section(section, section_url, start_page, end_page, db, log_fn=None, progress_fn=None, 
                        stop_flag=None, discount_alert_cb=None, concurrency=6, discount_threshold=30):
    """سكرابة قسم كامل"""
...
def start_scraping():
    ...
    pages = int(pages_entry.get()) if not all_pages else 100
    ...
    log(f"🚀 Starting - New Products: {auto_mode}, Price Check: {comparison_mode}")
...
def show_stats():
    ...
    if price_comparison_enabled[0]:
        ...
        log(f"   ✅ Found Matches: {stats['successful_comparisons']}")
        log(f"   📱 Sent Alerts: {stats['validated_deals']}")
...
title_label = ctk.CTkLabel(root, text="LAQTA - PRICE CHECKER", font=("SST Arabic Medium", 55), text_color="#54fac8")
title_label.grid(row=0, column=0, padx=8, pady=(15, 5), sticky="ew")

subtitle_label = ctk.CTkLabel(root, text="🔍 مقارنة الأسعار مع جوميا ونون للتأكد من العروض الحقيقية", 
...
all_pages_chk = ctk.CTkCheckBox(controls_frame, text="All Pages", font=("Arial", 12), text_color="#59ff9d")
all_pages_chk.grid(row=0, column=3, padx=4, pady=8, sticky="ew")
...
auto_new_chk.grid(row=0, column=4, padx=4, pady=8, sticky="ew")
auto_new_chk.select()

# مقارنة الأسعار (الميزة الرئيسية)
price_comparison_chk = ctk.CTkCheckBox(controls_frame, text="🔍 Price Check", font=("Arial", 12, "bold"), 
                                      text_color="#00aaff", command=toggle_price_comparison)
price_comparison_chk.grid(row=0, column=5, padx=4, pady=8, sticky="ew")
...
telegram_checkbox.grid(row=0, column=6, padx=4, pady=8, sticky="ew")
...
min_discount_slider.grid(row=0, column=7, padx=4, pady=8, sticky="ew")

min_discount_label = ctk.CTkLabel(controls_frame, text=f"Min: {ALERT_DISCOUNT}%", font=("Arial", 11), text_color="#59ff9d")
min_discount_label.grid(row=0, column=8, padx=4, pady=8, sticky="ew")
...
log_textbox = ctk.CTkTextbox(root, font=("Consolas", 12), fg_color="#20242f", text_color="#c2ffe3", border_width=0, height=220)
...
log("🔍 Price Check: ON - سيتم مقارنة الأسعار مع جوميا", "✨")
log("🆕 New Products: ON - منتجات جديدة فقط", "💡")
log("📱 Expected: Only VERIFIED deals with external price comparison!", "🏆")
